...
const QUICK_ACTIONS = [
  ...
  { label: '💎 Stake tokens',      prompt: 'How can I stake my B2S tokens to earn rewards?' },
  { label: '🔄 Swap tokens',       prompt: 'I want to swap STX for B2S tokens' },
];
...
export default function AgentChat() {
  const [messages, setMessages]   = useState<Message[]>([
    {
      role:      'agent',
      content:   '👋 Hey! I\'m **B2S Agent**, your AI-powered blockchain assistant.\n\nI can help you:\n• 💰 Manage your wallet & check balances\n• 🔄 Execute swaps and transfers\n• 🏅 Buy/sell NFT badges on the marketplace\n• 🌉 Track bridge transactions\n• 📊 Analyze DeFi strategies\n\nWhat would you like to do?',
      timestamp: new Date(),
    },
  ]);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      fetch('/api/agent')
        .then(r => r.json())
        .then(d => d.address && setWalletAddr(d.address))
        .catch(() => {});
    }
  }, [isOpen]);
    ...
    try {
      ...
      setMessages(prev => {
        ...
        updated[updated.length - 1] = {
          ...agentMsg,
          content: '❌ Agent error. Please check your API keys in `.env.local`.',
        };
        return updated;
      });
    } finally {
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs truncate" style={{ color: '#64748b' }}>
                {walletAddr ? `${walletAddr.slice(0, 8)}...${walletAddr.slice(-4)}` : 'AI-powered blockchain assistant'}
              </div>
            </div>
            ...
            {messages.map((msg, i) => (
              ...
              >
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: formatContent(msg.content) || (loading && i === messages.length - 1 ? '<span class="opacity-50">●●●</span>' : '') }}
                />
              </div>
            ))}
