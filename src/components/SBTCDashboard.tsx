...
interface SBTCDashboardProps {
  userAddress?: string
}

export function SBTCDashboard({ userAddress }: SBTCDashboardProps) {
  const [stats, setStats] = useState<SBTCStats>({
    totalSupply: 0,
    holders: 0,
    price: 0,
    volume24h: 0,
  ...
  const fetchStats = useCallback(async () => {
    try {
      const [supplyRes, holdersRes, priceRes] = await Promise.all([
      ...
      const [supplyData, holdersData, priceData] = await Promise.all([
        supplyRes.json(),
        holdersRes.json(),
        priceRes.json(),
      ])
      ...
      setStats({
        totalSupply: supplyData?.total_supply ? Number(supplyData.total_supply) / 1e8 : 0,
        holders: holdersData?.total || 0,
        price: priceData?.bitcoin?.usd || 66000,
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div>
          <div style={{ fontSize: '10px', letterSpacing: '0.3em', color: '#f7931a', marginBottom: '4px' }}>
            // SBTC_DASHBOARD
          </div>
          ...
          <div style={{ fontSize: '12px', color: '#64748b', marginTop: '2px' }}>
            Native Bitcoin bridged to Stacks mainnet
          </div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: '9px', color: '#64748b', letterSpacing: '0.2em' }}>BTC PRICE</div>
          <div style={{ fontSize: '24px', fontWeight: 700, color: '#f7931a' }}>
      ...
      <div style={{ background: 'rgba(247,147,26,0.05)', border: '1px solid rgba(247,147,26,0.15)', borderRadius: '2px', padding: '16px', marginBottom: '16px' }}>
        <div style={{ fontSize: '10px', letterSpacing: '0.3em', color: '#f7931a', marginBottom: '12px' }}>
          // BRIDGE_BTC_TO_SBTC
        </div>
        <div style={{ fontSize: '12px', color: '#94a3b8', marginBottom: '12px', lineHeight: 1.6 }}>
          Bridge your BTC to sBTC on Stacks mainnet. sBTC is fully backed 1:1 by Bitcoin and enables DeFi on Bitcoin.
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
          ...
          >
            ...
            href="https://explorer.hiro.so/address/SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token?chain=mainnet"
            target="_blank"
            rel="noopener noreferrer"
            style={{
              display: 'block',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', marginBottom: '16px' }}>
        {[
          { title: 'DECENTRALIZED', desc: 'No custodian — BTC locked in threshold wallet' },
          { title: 'TRUSTLESS', desc: 'Smart contract enforced 1:1 BTC backing' },
          { title: 'COMPOSABLE', desc: 'Use in DeFi, AMMs, lending protocols' },
        ].map((item) => (
          <div key={item.title} style={{
            ...
            <div style={{ fontSize: '10px', fontWeight: 700, color: '#f7931a', marginBottom: '4px' }}>{item.title}</div>
            <div style={{ fontSize: '10px', color: '#64748b', lineHeight: 1.5 }}>{item.desc}</div>
          </div>
        ))}
      </div>

      <div style={{ fontSize: '9px', color: '#1e3a2f', letterSpacing: '0.15em', textAlign: 'right' }}>
        LAST_SYNC: {lastSync} · CONTRACT: SM3VDXK3...sbtc-token
      </div>
    </div>
  )
}
